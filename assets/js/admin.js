...
function nextArt(num){
  firebaseRef.child("artworks").child("art"+num).on("value", function(snapshot){
    artposts.push({"image":snapshot.val()["image"], "picture":snapshot.val()["picture"], list:{
...
function nextSArt(num){
  firebaseRef.child("startworks").child("art"+num).on("value", function(snapshot){
    startposts.push({"image":snapshot.val()["image"], "list":{
    "date": snapshot.val().list.date,
    "desc": snapshot.val().list.desc,
    "name": snapshot.val().list.name,
    "media": snapshot.val().list.media,
...
function deletePost(){
  ...
  if(ctype=="artwork"){
    ...
    dpost=artposts[length+1];
    artSettingd(id);
    firebaseRef.child("tools").child("artworkC").set(length);
    artSettingd(0);
    return 0;
  } 
  if(ctype=="startwork"){
    length=startworkposts.length;
    dpost=startposts[length+1];
    stSettingd(id);
    firebaseRef.child("tools").child("startworkC").set(length);
    stSettingd(0);
    return 0;
  }
  if(ctype=="uartwork"){
    length=uartworkposts.length;
    dpost=uartposts[length+1];
    uSettingd(id);
    firebaseRef.child("tools").child("uartworkC").set(length);
    uSettingd(0);
    return 0;
  }
  if(ctype=="news"){
    ...
    dpost=news[length+1];
    nSettingd(id);
    firebaseRef.child("tools").child("nartworkC").set(length);
    nSettingd(0);
    return 0;
  }
}
