...
function readComments(){
  firebaseRef.child("tools").child("ncommentC").on("value", function(snapshot){
    let t=snapshot.val();
    ccount = t;
    for(i=0; i<t; i++){
      comments.push([]);
...
function nextComment(num){
  firebaseRef.child("ncomments").child("c"+num).on("value", function(snapshot){
    if(cdone==true){
...
function makePage(){
  ...
  for(i=0; i<posts.length; i+=2){
    ress[i]=false;
    ress[i+1]=false;
    let post1=createPost(posts[i]["image"], setPoster
    (posts[i]["list"]), i, posts[i]["list"]["name"]);
    let post2 = "";
...
function createPost(image, data, id, artname){
  return '<div><div class="box"><div class="image fit"></div><div class="content"><header class="align-center"><h2>'+artname+'</h2></header>'+data+'<footer class="align-center"><a style="margin-top: -10px;" id="ct'+id+'" class="button alt" onclick="comment('+id+');">Browse Comments</a></footer></div> <!-- content --></div> <!-- box --></div> <!-- empty -->'
}
...
function setPoster(list){
  let element = "";
  element="<p style='text-align: center;' >"+list["desc"]+"<br/><i>* This post was posted by Mark Talib on "+list["date"]+" *</i></p>";
  return element;
}
...
function addComment(num, textw, namew){
  firebaseRef.child("ucomments").child("c"+(ccount+1)).set({
    post: num,
    text: namew+" : "+textw
  });
  firebaseRef.child("tools").child("ucommentC").set(ccount+1);
  location.reload();
}
