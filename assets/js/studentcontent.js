...
function readComments(){
  firebaseRef.child("tools").child("stcommentc").on("value", function(snapshot){
    let t=snapshot.val();
    ccount = t;
    for(i=0; i<t; i++){
      comments.push([]);
...
function nextComment(num){
  firebaseRef.child("stcomments").child("c"+num).on("value", function(snapshot){
    if(cdone==true){
...
function createPost(image, data, id, artname){
  return '<div><div class="box"><div class="image fit"><img src="'+image+'" alt="" /></div><div class="content"><header class="align-center"><p>Available in Mid-High Resolution</p><h2>'+artname+'</h2></header>'+data+'<footer class="align-center"><a style="margin-top: -10px;" id="ct'+id+'" class="button alt" onclick="comment('+id+');">Browse Comments</a></footer></div> <!-- content --></div> <!-- box --></div> <!-- empty -->'
}
...
function setData(list){
  ...
  element="<p style='text-align: center;' >This artwork is named "+list["name"]+". It was created by "+list["author"]+". The medium of the artwork is '"+list["media"]+"'. This artwork was completed in "+list["date"]+" . "+list["desc"]+"</p>";
  return element;
}

function setPoster(list){
  let element = "";
  element="<p style='text-align: center;' >This artwork is from the upcoming artwork gallery, this artwork is named "+list["name"]+". It is being created by "+list["author"]+". The medium of the artwork is '"+list["media"]+"'. This artwork will soon be completed. "+list["desc"]+". Disclaimer: The image above is just a placeholder image. </p>";
  return element;
}
...
function addComment(num, textw, namew){
  firebaseRef.child("stcomments").child("c"+(ccount+1)).set({
    post: num,
    text: namew+" : "+textw
  });
  firebaseRef.child("tools").child("stcommentc").set(ccount+1);
  location.reload();
}
